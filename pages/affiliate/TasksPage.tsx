...
const TasksPage: React.FC = () => {
    ...
    if (loading) {
        return <p className="p-4 text-center text-text-secondary">Loading your tasks...</p>;
    }
    ...
    return (
        <div className="p-4 space-y-4">
            ...
            {!showOnboarding && (
              <>
                <h2 className="text-xl font-bold text-text-primary">Pending Tasks</h2>
                
                {tasks.length === 0 ? (
                    <Card>
                        <CardContent className="text-center py-10 flex flex-col items-center">
                            <CheckCircleIcon className="h-12 w-12 text-green-500" />
                            <h3 className="mt-4 text-lg font-semibold text-text-primary">All Caught Up!</h3>
                            <p className="mt-2 text-sm text-text-secondary">You have no pending tasks. Check the Campaigns tab to request new samples.</p>
                        </CardContent>
                    </Card>
                ) : (
                                ...
                                <CardContent>
                                    <div className="flex items-start gap-4">
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <h3 className="font-bold text-text-primary line-clamp-2 mt-1">{task.campaign?.name}</h3>
