...
const CampaignsPage: React.FC = () => {
    ...
    return (
        <div className="p-4 space-y-4">
            <h2 className="text-xl font-bold text-text-primary">Partner Campaigns</h2>
            <p className="text-xs text-center p-2 rounded-lg bg-primary/10 text-primary">
                💡 Remember to post one partner video weekly to stay eligible for new samples!
            </p>
            
            <div className="flex flex-col sm:flex-row gap-4">
                <div className="flex-1">
                    <label htmlFor="sort" className="block text-sm font-medium text-text-secondary">Sort by</label>
                    <select id="sort" value={sortOrder} onChange={(e) => setSortOrder(e.target.value as any)} className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-border bg-surface rounded-md focus:outline-none focus:ring-primary focus:border-primary sm:text-sm">
                 ...
                 <div className="flex-1">
                    ...
                    <select id="category" value={selectedCategory} onChange={(e) => setSelectedCategory(e.target.value)} className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-border bg-surface rounded-md focus:outline-none focus:ring-primary focus:border-primary sm:text-sm">
                ...
                return (
                    <Link to={`/campaign/${campaign.id}`} key={campaign.id} className="block hover:opacity-90 transition-opacity" data-testid={`campaign-card-link-${campaign.id}`}>
                        <Card className="relative overflow-hidden h-full flex flex-col">
                            ...
                            {request && (
                                <span className="absolute top-1 right-1 bg-secondary text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full z-10">
                            ...
                            <CardContent className="flex-1 flex flex-col justify-between p-3">
                                <div>
                                    <h3 className="text-sm font-bold text-text-primary line-clamp-2">{campaign.name}</h3>
                                </div>
                                <div className="mt-2 text-right">
                                     <p className="text-sm font-bold text-primary">{campaign.commission}%</p>
                                </div>
                            </CardContent>
                        </Card>
                    </Link>
