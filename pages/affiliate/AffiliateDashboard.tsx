...
const AffiliateDashboard: React.FC = () => {
    ...
    if (currentPath === 'campaign' || TABS.map(t => t.id).includes(currentPath as AffiliateTab)) {
        ...
        if (currentPath.startsWith('rewards')) activeTabId = 'rewards';
    } else {
    ...
    return (
        <div className="h-screen w-screen max-w-md mx-auto flex flex-col bg-background font-sans">
            {isSurveyModalOpen && <WeeklySurveyModal onClose={() => setIsSurveyModalOpen(false)} />}
            
            <header className="p-4 bg-surface/50 sticky top-0 z-10 backdrop-blur-sm border-b border-border">
            ...
            {/* Bottom Tab Bar */}
            <nav className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-surface/80 backdrop-blur-sm border-t border-border flex justify-around">
                {TABS.map(tab => (
                    <button
                        key={tab.id}
                        ...
                        className={`flex flex-col items-center justify-center w-full pt-2 pb-1 text-xs font-medium transition-colors duration-200 ${
                            activeTabId === tab.id ? 'text-primary' : 'text-text-secondary hover:text-primary'
                        }`}
                    >
