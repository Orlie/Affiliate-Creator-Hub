...
const ResourcesPage: React.FC = () => {
    ...
    return (
        <div className="p-4">
            <h2 className="text-xl font-bold text-text-primary text-center">Tips & Tricks</h2>
                    ...
                    filteredResources.length > 0 ? filteredResources.map(article => (
                        <Card key={article.id}>
                            <CardContent>
                                ...
                                <p className="mt-2 text-sm text-text-secondary">{article.content}</p>
                            </CardContent>
                        </Card>
                    )) : <p className="text-center text-text-secondary">No resources found in this category.</p>
                }
            </div>
        </div>
    );
